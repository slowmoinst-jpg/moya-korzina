...
# ---------- фасовка как ключ сопоставления ----------
def test_same_word_different_pack_is_a_different_product():
    """Самая дорогая ошибка подбора за сессию — и её ключ.

    «Страчателла 200 г» сопоставилась с мороженым «страчателла 92 г»: слово
    вошло целиком, значит по прежнему правилу «это оно». Фасовка снимает вопрос
    без всякой похожести — двести грамм и девяносто два это разные товары.
    """
    assert not compare.is_relevant("Страчателла 200 г",
                                   "Мороженое Maxibuo брикет страчателла 92 г")
    assert compare.is_relevant("Страчателла 200 г", "Сыр Страчателла мягкий 200г")


def test_weight_beats_word_containment():
    """Вхождение всех слов больше не перебивает несовпадение фасовки."""
    # все слова запроса есть в названии — прежнее правило сказало бы «берём»
    assert not compare.is_relevant("Икра лососевая 180 г", "Икра лососевая отборная 500 г")


def test_missing_weight_is_not_evidence():
    """Молчание не улика: нет граммовки — не возражаем.

    У весовых товаров её нет ни у кого («Слива»), и отсекать по ней значит
    выбросить их целиком.
    """
    assert compare.is_relevant("молоко", "Молоко пастеризованное 930 мл")
    assert compare.is_relevant("Слива", "Слива отборная")
    assert compare.weights_agree("Огурцы", "Огурцы короткоплодные")


def test_tolerance_allows_honest_rounding():
    """Допуск нужен: 930 мл и 1 л — часто один и тот же товар в разных описаниях."""
    assert compare.weights_agree("Молоко 1 л", "Молоко 930 мл"), "7% — в пределах допуска"
    assert not compare.weights_agree("Молоко 1 л", "Молоко 500 мл"), "вдвое меньше — другой товар"


def test_broader_candidate_is_not_the_same_product():
    """Кандидат короче запроса — товар шире, а не тот же.

    На «Персики консервированные 500 г» Магнит отдавал просто «Персики» —
    свежие, 107,99 ₽ против 418 у консервированных. Похожесть подскакивала до
    0,9, потому что короткое название входит в длинный запрос, и вхождение
    засчитывалось за попадание.
    """
    assert not compare.is_relevant("Персики консервированные 500 г", "Персики")
    assert not compare.is_relevant("Огурцы короткоплодные 450 г", "Огурцы")


def test_direction_of_containment_decides():
    """Искали общее, нашли частное — это оно. Наоборот — нет."""
    assert compare.is_relevant("молоко", "Молоко пастеризованное 930 мл"), \
        "уточнения ДОБАВИЛИСЬ — тот же товар"
    assert not compare.is_relevant("молоко пастеризованное", "Молоко"), \
        "уточнение ПОТЕРЯЛОСЬ — товар шире"


def test_numbers_are_not_lost_qualifiers():
    """Цифры и единицы не считаются потерянным уточнением: фасовку проверяет вес."""
    assert not compare.is_broader("Молоко 930 мл", "Молоко 930 мл")
    assert not compare.is_broader("Сыр 200 г", "Сыр")  # «200» и «г» выброшены, слова равны
