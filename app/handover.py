...
от «вся корзина одной ссылкой» до «вот список, дальше руками».
    ...
    LINK   вся корзина уезжает одной ссылкой.
           ВкусВилл: инструмент vkusvill_cart_link_create его собственного MCP.
    ...
    ITEMS  ссылки на карточки товаров. На телефоне каждая открывается прямо в
           ...
           у Магнита разрешён путь /product/*, у Ленты тоже product/*. Шестнадцать
           позиций — шестнадцать нажатий, некрасиво, но работает и ничего чужого
           не требует.

    LIST   магазин не принимает ничего. Пятёрочка закрыта геоблоком и отвечает 403
           на всё, Дикси показывает капчу. Остаётся список, который человек
           открывает рядом с их приложением и отмечает по мере сборки.
