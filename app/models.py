...
class Location:
    """Где находится клиент. Параметр запроса, а не настройка приложения.

    Цена и наличие у сетей свои в каждой точке, и различие не косметическое:
    у Ленты одно и то же молоко стоит 90,99 ₽ на Ходынском бульваре и 91,99 ₽
    в Екатеринбурге, причём различается даже глубина акции (−27 % против −16 %).
    У Магнита из одного и того же адреса коктейль стоит 175,99 ₽ в Краснодаре
    и 169 ₽ в Зеленограде, а молока «Кубанский молочник» в московском магазине
    нет вовсе. Значит спрашивать цены «вообще» нельзя — только за конкретного
    человека по его адресу.

    Пустое место (без адреса и без кода точки) ложно: коннектор по нему видит,
    что спрашивать нечего, и берёт запасное значение из config.yaml.
    """
    address: str | None = None       # «Москва, Ходынский бульвар 4»
    store_id: str | None = None      # код точки; надёжнее адреса — его сервер разбирает сам
    shop_type: str | None = None     # формат точки у Магнита: ME, MM, DG, GM, MC…
    delivery: bool = True            # цены доставки или самовывоза — они разные

    def __bool__(self) -> bool:
        return bool(self.address or self.store_id)

    @property
    def key(self) -> str:
        """Часть ключа кэша.

        Без неё цены одной точки достались бы клиенту другой — молча и
        правдоподобно, потому что цена чужого города выглядит как настоящая.
        """
        return "|".join((
            str(self.store_id or ""),
            str(self.address or ""),
            str(self.shop_type or ""),
            "d" if self.delivery else "p",
        ))
