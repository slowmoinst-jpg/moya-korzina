...
def _frame() -> None:
    ...
    st.caption("Войдите в окне. **В списке чеков видны только суммы — позиций там нет**, "
               "поэтому откройте нужный чек: внутри будут товары. Их и выделяйте "
               "(Ctrl+A), копируйте (Ctrl+C) и вставляйте в поле под окном, не уходя "
               "с этой страницы. Кнопки «загрузить само» здесь нет намеренно: браузер "
               "не даёт странице читать содержимое чужого сайта, и такая кнопка была "
               "бы обманом.")
